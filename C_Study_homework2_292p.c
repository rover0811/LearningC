...
{ 
	...
	while(number1<6)                 // 7번째 원소를 받기 전까지 while문을 실행시켜 모든 로또 번호 입력 받기  
	{
		...
		scanf("%d",&lotto_nums[number1]);
		if(lotto_nums[number1]>45 || lotto_nums[number1]<1) // 주어진 범위 밖의 수를 입력하면 다시 숫자 받기  
		{
			printf("번호를 다시 입력하세요!\n");
			continue;
		}
		...
		{
			if(lotto_nums[i] == lotto_nums[number1])
			{
				printf("같은 번호가 있습니다!\n");
				number2++;
				break;
		...
		if(number1 != number2)     // 이 조건문이 참이면 곧 같은 원소가 있다는 뜻이므로 continue
			continue; //같은 원소가 없으면 number1의 인덱스 원소에 입력 받은 수 대입  
		number1++;
		number2++;
	}
}
